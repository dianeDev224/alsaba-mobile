...
import { AlsabaService } from "../../app.data";

const ServiceMenuPopup = memo(function ({ isModalVisible, setModalVisible }) {
    const toggleModal = () => {
                ...
                >
                    <StyledView className={`flex flex-col w-[95%] ${styleSheet.popupContainerWidth} h-[80%] bg-[#fff] rounded-lg p-2`}>
                        <StyledView className='flex flex-col w-full space-y-[20px] justify-center items-center '>
                            ...
                            {AlsabaService.map((item) => {
                                return (
                                    <StyledView key={item.id} className='flex flex-row w-[200px] h-[40px] ml-[5px] bg-[#48d351] items-center justify-center rounded-2xl'>
                                        <item.IconValue name={item.IconName} size={20} color="#115DA9" />
                                        <Text className="font-bold uppercase italic ml-[5px] text-[#115DA9]">{item.Name}</Text>
                                    </StyledView>
                                )
                            })}
                        </StyledView>
...
export default ServiceMenuPopup