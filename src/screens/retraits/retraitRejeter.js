...
export default function RetraitRejeter({ itemName = "Transfert Annulés" }) {
    ...
    return (
        <StyledView className='flex-1 bg-[#f5e2e6]'>
            <StyledView className="flex flex-col item-center justify-center w-full">
                <Text className={`w-full font-bold text-[15px] italic text-[#94162b] text-center my-2`}>Retraits Rejetés</Text>
                ...
                <StyledView>
                    ...
                    <StyledView className='flex flex-row w-full justify-center items-center h-[40px]'>
                        ...
                        <Text className={`font-bold text-[20px] bg-[#94162b] text-[#fff]`}>{currentPage}</Text>
    ...
    return (
        <Modal isVisible={show} className={`flex flex-col b-[#ffa] self-center flex-1`}>
            <OutsidePressHandler className="bg-[#fff] flex flex-col basis-[250px] w-[300px] space-y-5 p-4" onOutsidePress={() => toggleShow(false)}>
                <StyledView className="flex flex-row items-center justify-center w-[200px] h-[40px] bg-[#94162b] self-center">
...
const styles = StyleSheet.create({
    ...
    head: {
        height: 40, backgroundColor: "#94162b", textAlign: 'center', fontWeight: 'bold'
    },
    headText: {
        textAlign: 'center',
        color: "#fff",
