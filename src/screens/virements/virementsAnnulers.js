...
export default function VirementAnnulers({ itemName = "Transfert Annulés" }) {

    const StyledView = styled(View)
    const windowSize = useWindowDimensions();
    const styleSheet = styleGenerator({ windowHeight: windowSize.height, windowWidth: windowSize.width })
    const tableHead = ['Date', 'Beneficiaire', 'Montant', 'Action'];
    const tableData = [
        ['1', '2', '3', '4'],
        ['a', 'b', 'c', 'd'],
        ['1', '2', '3', '456\n789'],
    ...
    return (
        <StyledView className="flex-1 bg-[#ebd4ca]">
            <StyledView className="flex flex-col item-center justify-center w-full">
                <Text className={`w-full font-bold text-[15px] italic text-[#af5633] text-center my-2`}>Virements Annulers</Text>
                ...
                <ScrollView>
                    ...
                    <StyledView className='flex flex-row w-full justify-center items-center h-[40px]'>
                        ...
                        <Text className={`font-bold text-[20px] bg-[#af5633] text-[#fff]`}>{currentPage}</Text>
...
function actionButton(cellData, index, handleClick) {
    ...
    return (
        ...
        <Pressable className="flex-1 flex-row items-center justify-center w-full" onPress={handleClick}>
    ...
    return (
        <Modal isVisible={show} className={`flex flex-col b-[#ffa] self-center flex-1`}>
            <OutsidePressHandler className="bg-[#fff] flex flex-col basis-[250px] w-[300px] space-y-5 p-4" onOutsidePress={() => toggleShow(false)}>
                <StyledView className="flex flex-row items-center justify-center w-[250px] h-[40px] bg-[#af5633] self-center">
                    <Text className="text-[#fff] font-bold">Details Du Virement #ff4</Text>
                </StyledView>
                <StyledView className="flex flex-col flex-1 space-y-2">
                    <Text className="font-semibold">Montant: 200DH</Text>
                    <Text className="font-semibold">Beneficiaire : </Text>
                    <Text className="font-semibold">Motif : </Text>
                </StyledView>
...
const styles = StyleSheet.create({
    ...
    head: {
        height: 40, backgroundColor: "#af5633", textAlign: 'center', fontWeight: 'bold'
    },
    headText: {
        textAlign: 'center',
        color: "#fff",
