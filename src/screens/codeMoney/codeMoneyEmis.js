...
export default function CodeMoneyEmis({ itemName = "Transfert Annulés" }) {
    ...
    return (
        <StyledView className='flex-1 bg-[#f0efca]'>
            <StyledView className="flex flex-col item-center justify-center w-full">
                <Text className={`w-full font-bold text-[15px] italic text-[#706f1e] text-center my-2`}>Nouveau Emis</Text>
    ...
    return (
        <Modal isVisible={show} className={`flex flex-col b-[#ffa] self-center flex-1`}>
            <OutsidePressHandler className="bg-[#fff] flex flex-col basis-[250px] w-[300px] space-y-5 p-4" onOutsidePress={() => toggleShow(false)}>
                <StyledView className="flex flex-row items-center justify-center w-[200px] h-[40px] bg-[#706f1e] self-center">
...
const styles = StyleSheet.create({
    ...
    head: {
        height: 40, backgroundColor: "#706f1e", textAlign: 'center', fontWeight: 'bold'
    },
    headText: {
        textAlign: 'center',
        color: "#fff",
