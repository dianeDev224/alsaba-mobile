...
import MoreDetailsTransfertEmis
  from '../../components/popup/transfertConofirmation/moreDetailsTransfertEmis';

export default function TransfertEmis({ itemName = "Transfert Annulés" }) {
    ...
    return (
        <StyledView style={{ width: SCREEN_WIDTH, backgroundColor: "#e2c0da", flex: 1 }}>
            <StyledView className="flex flex-col item-center justify-center w-full">
                <Text className={`w-full font-bold text-[15px] italic text-[#7a2565] text-center my-2`}>Transfert Emis</Text>
                ...
                <StyledView>
                    ...
                    <StyledView className='flex flex-row w-full justify-center items-center h-[40px]'>
                        ...
                        <Text className={`font-bold text-[20px] bg-[#7a2565] text-[#fff]`}>{currentPage}</Text>
                        <MaterialIcons name="chevron-right" size={30} color="black" />
                    </StyledView>
                </StyledView>
            </StyledView>
            <MoreDetailsTransfertEmis isModalVisible={showDetails} setModalVisible={setShowDetails} />
        </StyledView>
    );
}
...
const styles = StyleSheet.create({
    ...
    head: {
        height: 40, backgroundColor: "#7a2565", textAlign: 'center', fontWeight: 'bold'
    },
    headText: {
        textAlign: 'center',
        color: "#fff",
