...
import MoreDetailsTransfertAnnuler
  from '../../components/popup/transfertConofirmation/moreDetailsTransfertAnnuler';

export default function TransfertSupprimer({ itemName = "Transfert Annulés" }) {
    ...
    return (
        <StyledView style={{ width: SCREEN_WIDTH, backgroundColor: "#f0cfc4", flex: 1 }}>
            <StyledView className="flex flex-col item-center justify-center w-full">
                <Text className={`w-full font-bold text-[15px] italic text-[#833e29] text-center my-2`}>{itemName}</Text>
                ...
                <StyledView>
                    ...
                    <StyledView className='flex flex-row w-full justify-center items-center h-[40px]'>
                        ...
                        <Text className={`font-bold text-[20px] bg-[#833e29] text-[#fff]`}>{currentPage}</Text>
                        <MaterialIcons name="chevron-right" size={30} color="black" />
                    </StyledView>
                </StyledView>
            </StyledView>
            <MoreDetailsTransfertAnnuler isModalVisible={showDetails} setModalVisible={setShowDetails} />
        </StyledView>
    );
}
...
const styles = StyleSheet.create({
    ...
    head: {
        height: 40, backgroundColor: "#833e29", textAlign: 'center', fontWeight: 'bold'
    },
    headText: {
        textAlign: 'center',
        color: "#fff",
