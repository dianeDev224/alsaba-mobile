...
export default function DepotAnnuler({ itemName = "Transfert Annulés" }) {
    ...
    return (
        <StyledView className='flex-1 bg-[#e7d2d7]'>
            <StyledView className="flex flex-col item-center justify-center w-full">
                <Text className={`w-full font-bold text-[15px] italic text-[#ab1639] text-center my-2`}>Depôts Annulés</Text>
                ...
                <ScrollView>
                    ...
                    <StyledView className='flex flex-row w-full justify-center items-center h-[40px]'>
                        ...
                        <Text className={`font-bold text-[20px] bg-[#ab1639] text-[#fff]`}>{currentPage}</Text>
    ...
    return (
        <Modal isVisible={show} className={`flex flex-col b-[#ffa] self-center flex-1`}>
            <OutsidePressHandler className="bg-[#fff] flex flex-col basis-[250px] w-[300px] space-y-5 p-4" onOutsidePress={() => toggleShow(false)}>
                <StyledView className="flex flex-row items-center justify-center w-[200px] h-[40px] bg-[#ab1639] self-center">
...
const styles = StyleSheet.create({
    ...
    head: {
        height: 40, backgroundColor: "#ab1639", textAlign: 'center', fontWeight: 'bold'
    },
    headText: {
        textAlign: 'center',
        color: "#fff",
